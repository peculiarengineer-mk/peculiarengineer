...
The first surprise was a 182 MB journal cap on a 38 GB disk. The size ceiling journald enforces is computed once, when the daemon starts, and on this cloud image that happened seven seconds before the root filesystem was grown. Then `MaxRetentionSec=`, which does not delete entries older than the limit, it deletes whole files whose oldest entry is older than the limit, and takes the newer entries in them along. Then rsyslog, which on this image keeps a second copy in `/var/log/syslog` and its siblings under a separate retention policy, so the journal is not the whole bill. And a `ForwardToSyslog=no` drop-in named the obvious thing is silently overruled by a file Ubuntu ships in `/usr/lib`.

> **TL;DR.** By default journald caps itself at 10% of the filesystem or 4 GB, whichever is smaller, keeps 15% free, and computes that once at startup, so restart it after growing a disk. To size it yourself, put `SystemMaxUse=`, `MaxRetentionSec=` and `MaxFileSec=` in `/etc/systemd/journald.conf.d/zz-retention.conf` and `sudo systemctl restart systemd-journald`; the restart deletes archives down to the cap immediately. Retention is enforced per file and deletes the whole file once its oldest entry expires, newer entries included, so a short `MaxFileSec=` is what keeps `MaxRetentionSec=` close to the number you set. rsyslog writes a second, truncated copy to `/var/log/syslog`; turn it off with `ForwardToSyslog=no` in a drop-in whose name sorts after `syslog.conf`, or remove rsyslog. `journalctl --disk-usage` does not count leftover journal directories from the image build; `-m` does.
...
So the ceiling is 10% of the filesystem holding `/var/log/journal`, up to 4 GB, and journald also refuses to eat into the last 15%, again up to 4 GB. (systemd's `M` and `G` are binary units, so its 4G is 4 GiB; I use the shorter form throughout.) Individual files are capped at one eighth of that, up to 128 MB, "so that usually seven rotated journal files are kept as history." Time based limits are off: `MaxRetentionSec=0`, and `MaxFileSec=1month` only forces a rotation every month. One more default worth a glance: the commented `#Storage=persistent` shows Ubuntu compiles journald with persistent storage, where upstream's default is `auto`. The earlier post said `auto`; the effect on this image is the same either way, because the on disk directory exists. journald prints the numbers it settled on every time it starts, which is the single most useful line in this whole topic:
...
That is the answer for this 38 GB disk: a 3.7 GB ceiling. For the disks you are likely to have, by the same formula (only the 38 GB row is measured, though the earlier post saw the 4 GB cap on a 75 GB disk):
...
$ ls -la /var/log/journal/2ec6eaf028db408b958d67c7632c9bd7/
-rw-r-----+ 1 root systemd-journal  50331648 system.journal
-rw-r-----+ 1 root systemd-journal 134217728 system@92f7...-0000000000000001-00065bea4df8d344.journal
-rw-r-----+ 1 root systemd-journal 134217728 system@92f7...-00000000000016fe-00065bea5851f61e.journal
```

Three things in that listing. 400 MB in became 304 MB on disk, because journald compresses entries with zstd and even random base64 shrinks by a quarter. The archived files are exactly 128 MB each, the file size cap from above, and `system.journal` is the active one journald is still writing. And the file names carry two hex numbers, the sequence number and the timestamp of the first entry in that file. `00065bea4df8d344` is microseconds since the epoch and decodes to 13:42:28 UTC, the first line of this boot. That timestamp is going to matter in the retention section.

`journalctl --header` shows the same files with their state, `ONLINE` for the active one and `ARCHIVED` for the rest, and per file usage. Also worth knowing:
...
If you read logs with `journalctl`, and after the earlier post you should, the text copy is dead weight. There are two ways to stop it and one trap. The trap first. Ubuntu turns forwarding on with a drop-in it ships in `/usr/lib/systemd/journald.conf.d/syslog.conf`, and drop-ins apply in filename order across directories, so a file called `50-retention.conf` in `/etc` loses to it:
...
Last one wins, so that `no` did nothing, and the test message still landed in `/var/log/syslog`. Two names work: `/etc/systemd/journald.conf.d/syslog.conf`, which replaces the shipped file outright because same named files in `/etc` mask `/usr/lib`, or anything that sorts after it, which is why the drop-in in this post is called `zz-retention.conf`. With either, the test message reached the journal and not the text file. `systemd-analyze cat-config` is how you check which setting won before you trust it.
...
On a Raspberry Pi or anything else that boots from flash, a journal writing to `/var/log/journal` all day is wear you do not need. `Storage=volatile` keeps it in memory under `/run/log/journal` instead, with its own smaller ceiling. Two caveats before the command. Forwarding to rsyslog carries on regardless, so on a box with rsyslog installed the text copy is still being written to flash unless you turn forwarding off or stop rsyslog. And if you already have the retention drop-in from this post, add `Storage=volatile` to it rather than replacing it, or the `ForwardToSyslog=no` line goes away with the file.
...
A gigabyte is a number I can reason about instead of a percentage that moves with the disk, and it is a starting point: after a week I check `--list-boots` and `--disk-usage` to see how much history the box's real workload leaves under it. `MaxRetentionSec=1month` with `MaxFileSec=1week` means old logs go a week at a time, so "a month" is three to four weeks of history and never less than three. `ForwardToSyslog=no` stops the second copy, and works only because the file sorts after `syslog.conf`. Then `sudo mkdir -p /etc/systemd/journald.conf.d` if it does not exist yet (it did not on my fresh box), `sudo systemctl restart systemd-journald`, and read the `System Journal` line it prints to confirm the cap you meant is the cap you got. On a VPS with a 20 GB disk I would halve the size. On a Pi, `Storage=volatile` and accept that the journal starts over on every boot.
...
- A `ForwardToSyslog=no` drop-in named `50-anything.conf` does nothing, because Ubuntu's `syslog.conf` in `/usr/lib` sorts after it. Name yours `syslog.conf` or `zz-something.conf`, and check with `systemd-analyze cat-config`.
...
EOF
sudo systemctl restart systemd-journald
systemd-analyze cat-config systemd/journald.conf | grep -vE '^#|^$'   # which value won
...
sudo rm -r /var/log/journal/<the other id>
...
The defaults are sane on a big disk and most people never look, which is fine right up to the day `df` says the journal has 4 GB and the boot you need is not in it. Set the lines, read the startup line back, and check what actually survived a week later.
