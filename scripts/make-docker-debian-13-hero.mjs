...
const badge = 'DEBIAN 13 · DOCKER';
const termTitle = 'trixie, not resolute';
const termLines = [
  {t:'apt-cache policy docker.io',p:1,c:'bright',size:15},{t:'  Candidate: 26.1.5+dfsg1-9+deb13u1',c:'dim'},
  {t:'apt-cache policy docker-ce',p:1,c:'bright',size:15},{t:'  Candidate: 5:29.8.0-1~debian.13~trixie',c:'good'},
  {t:'',c:'dim'},{t:'# /etc/apt/sources.list.d/docker.sources',c:'dim'},
  {t:'URIs: https://download.docker.com/linux/debian',c:'bright'},{t:'Suites: trixie',c:'accent'},{t:'Signed-By: /etc/apt/keyrings/docker.asc',c:'bright'},
  {t:'',c:'dim'},{t:'docker run --rm -v /:/host alpine head -1 /host/etc/shadow',p:1,c:'bright'},{t:'root:!:17478:0:99999:7:::   # as a non-sudo user',c:'bad'}
];
const rightTitle = 'ARCHIVE VS DOCKER REPO';
const rightRows = [
  {k:'docker.io in trixie',v:'26.1.5, frozen for the release',c:'dim'},
  {k:'docker-ce from download.docker.com',v:'29.8.0 + compose v5 + buildx',c:'good'},
  {k:'the repo file',v:'deb822 .sources, Suites: trixie'},
  {k:'the docker group',v:'is root. decide on those terms.',c:'bad'}
];
const footer = 'same five packages as Ubuntu. different URL, different codename.';
const closer = 'one group away from root.';
...
writeFileSync(new URL('../src/assets/.docker-debian-13-hero.svg', import.meta.url), svg);
await sharp(Buffer.from(svg), { density: 144 }).resize(W * 2, H * 2).png()
  .toFile(new URL('../src/assets/docker-debian-13-hero.png', import.meta.url).pathname);
console.log('wrote src/assets/docker-debian-13-hero.png');
