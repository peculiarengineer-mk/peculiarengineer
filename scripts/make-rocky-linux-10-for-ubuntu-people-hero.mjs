...
const badge = 'ROCKY 10 · FOR UBUNTU PEOPLE'; const bw = (badge.length + 2) * 13.2 + 52;
const esc = s => s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
const rows = [
  { k: 'kernel',   d: '6.12',              u: '7.0' },
  { k: 'packages', d: 'dnf + EPEL',        u: 'apt + PPAs' },
  { k: 'admin',    d: 'wheel',             u: 'sudo' },
  { k: 'network',  d: 'NetworkManager',    u: 'netplan + networkd' },
  { k: 'firewall', d: 'firewalld (absent)',u: 'ufw' },
  { k: 'mac',      d: 'SELinux, permissive', u: 'AppArmor' },
  { k: 'pip lock', d: 'none',              u: 'EXTERNALLY-MANAGED' },
];
const term = [
  ['$','apt install htop','bright'],['','apt: command not found','bad'],
  ['$','dnf install htop','bright'],['','No matching Packages to list','bad'],
  ['$','usermod -aG sudo keith','bright'],['','group \'sudo\' does not exist','bad'],
  ['$','setenforce 1 && systemctl restart sshd','bright'],['','Bind to port 2222 failed: Permission denied','bad'],
  ['$','curl 127.0.0.1:8080','bright'],['','502  (13: Permission denied) upstream','bad'],
  ['','','dim'],
  ['','journalctl, systemd, chrony: identical.','good'],
];
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <text x="200" y="156" font-family="${mono}" font-size="20" fill="${dim}">ubuntu habits, rocky prompt</text>
  ...
  <text x="104" y="548" font-family="${mono}" font-size="16" fill="${dim}">same journal, different everything else.<tspan fill="${bright}">&#9608;</tspan></text></g>
  <g font-family="${mono}"><text x="852" y="200" font-size="20" letter-spacing="2" fill="${dim}">SAME PROVIDER, SAME DAY</text>
  <text x="1000" y="232" font-size="13" fill="${dim}">rocky 10</text><text x="1180" y="232" font-size="13" fill="${dim}">ubuntu 26.04</text>
  ...
  <text x="852" y="${262+7*44+8}" font-size="13" fill="${accent}">and SELinux is permissive until the day you flip it.</text></g>
</svg>`;
writeFileSync(new URL('../src/assets/.rocky-linux-10-for-ubuntu-people-hero.svg', import.meta.url), svg);
await sharp(Buffer.from(svg), { density: 144 }).resize(W*2, H*2).png().toFile(new URL('../src/assets/rocky-linux-10-for-ubuntu-people-hero.png', import.meta.url).pathname);
