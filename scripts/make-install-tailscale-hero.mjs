...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <!-- badge -->
  <g>
    <rect x="1104" y="48" width="208" height="46" rx="8" fill="none" stroke="${dim}" stroke-width="1.5"/>
    <text x="1130" y="77" font-family="${mono}" font-size="20" letter-spacing="2" fill="${bright}">&#9670; TAILSCALE</text>
  </g>
  ...
  <g>
    ...
    <text x="200" y="156" font-family="${mono}" font-size="20" fill="${dim}">ubuntu 24.04 &#183; joining the tailnet</text>
...
writeFileSync(new URL('../src/assets/.install-tailscale-hero.svg', import.meta.url), svg);
await sharp(Buffer.from(svg), { density: 144 })
  .resize(W * 2, H * 2)
  .png()
  .toFile(new URL('../src/assets/install-tailscale-hero.png', import.meta.url).pathname);
console.log('wrote src/assets/install-tailscale-hero.png');
